...
`build:auth` / `test:auth` are green (36 tests). Expired-session cleanup runs
...
- Service: family-scoped rotation/resolve/reuse-revoke, `auth:revoked-family:*`
  ...
  Bearer-access-token→refresh-cookie→revoke-all fallback, opportunistic
  `deleteExpiredSessions()` on login.
- Docs: logout / session-management / single-vs-multi-device notes corrected.

Prerequisite: `AUTH_FIX_PLAN.md` has been applied. It deleted dead token-session
code, fixed Bearer/cookie resolution priority, and hardened
`markPreviousUsed` — all of which shrank this plan's surface area.
...
- [x] Update `src/schema/sqlite.ts` and `src/schema/pg.ts` (the only two
...
- [x] Make `tokens.tokenFamily` required and unique. It is already generated as
      `nanoid(16)` and preserved across refresh rotations.
- [x] Add a non-unique `tokens_user_id_idx` for revoke-all, cleanup, and future
      session listing.
- [x] Keep `tokens_expires_at_idx`.
...
- [x] Add `tokenFamily` to refresh JWT payloads in `signRefreshToken`.
- [x] Require `tokenFamily` when verifying refresh tokens.
- [x] Add `tokenFamily` to access JWT payloads so current-session revocation can
      reject every access token minted for that family, not just the one Bearer
      token passed to logout.
- [x] Update `JwtPayload` in `src/types.ts` with optional `tokenFamily` for
      access tokens and required handling for refresh-token verification.

## Repository Work

- [x] Change `TokenRepository.storeRefreshToken` to upsert on
...
- [x] Replace `getRefreshTokenWithFamily(userId)` with lookup by
      `(userId, tokenFamily)` or by `tokenFamily` plus a user-id assertion from
      the verified JWT.
- [x] Change `markPreviousUsed` to key on `tokenFamily` instead of `userId`,
...
- [x] Split revocation APIs into `revokeFamily(tokenFamily)` and
      `revokeAllForUser(userId)`.
- [x] Do not keep a `revokeByFamily` alias; `revokeFamily` is the implemented
      public/internal API name.
- [x] Add an expiry cleanup path: with one row per family and no unique
...
- [x] Pass the family into both access and refresh signing.
- [x] Store the hashed refresh token under that family. The previous-hash
...
- [x] In `refreshTokens`, verify the refresh JWT, read `{ userId, tokenFamily }`,
...
- [x] In `resolveUserFromCookie`, resolve by the refresh token's family instead
      of selecting the user's single token row.
- [x] On refresh-token reuse outside the grace window, revoke only the suspect
...
- [x] Add a short-lived cache marker such as `auth:revoked-family:<tokenFamily>`
      for the access-token TTL when a family is revoked.
- [x] In `verifyAccessToken`, check both the existing user session-version key
      and the family revocation marker when `payload.tokenFamily` exists.
- [x] On normal logout, revoke only the current family, blacklist the presented
      access token when available, clear cookies, and do not invalidate every
      session for the user.
- [x] Define where logout obtains the family: prefer the verified Bearer access
      token's `tokenFamily` claim; fall back to a verified refresh cookie that
      still matches its family row. If neither is available, fall back to
      revoke-all for that user.
- [x] On password change, password reset, and future "logout all devices", keep
...
- [x] Preserve the documented session-cookie lag: other devices may keep the
...
- [x] Run `bun run test:auth`. (36 pass)

## Acceptance

- [x] Two independent cookie sessions for the same user can log in, refresh, and
      call `/auth/me` without invalidating each other.
- [x] Revoking or detecting reuse in one family does not delete another family's
      refresh row.
- [x] Normal logout affects only the current session.
- [x] Password change/reset still force all sessions to re-authenticate.
- [x] Existing security behavior remains covered by focused auth tests.
