...
export const CheckboxInput: React.FC<CheckboxInputProps> = ({ value, onChange, helper, label, checkboxClassName, className, variant = "default", status = "default", bordered, borderColor }) => (
  <BaseInput variant={variant} status={status} bordered={bordered} borderColor={borderColor} className={cn("flex gap-2 items-center", className)}>
