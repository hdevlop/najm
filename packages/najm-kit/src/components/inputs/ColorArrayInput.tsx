...
export const ColorArrayInput: React.FC<ColorArrayInputProps> = ({ value, onChange, className = "", variant = "default", status = "default", bordered, borderColor, colors = DEFAULT_COLORS }) => {
  const [selectedColor, setSelectedColor] = useState<string>(value || "");

  return (
    <BaseInput variant={variant} status={status} bordered={bordered} borderColor={borderColor} className={cn("flex flex-wrap gap-2", className)}>
      {colors.map((color) => (
