...
describe("data-card-shell.test.tsx", () => {
  ...
  test("selected row has ring-primary class when card is not bordered", async () => {
    const row = {
      id: "1",
      original: { id: "1", name: "Alice" },
      getIsSelected: () => true,
    ...
    const card = container.querySelector("[data-row=true]");
    expect(card).toBeTruthy();
    expect(card?.className).toContain("ring-2");
    expect(card?.className).toContain("ring-primary");
  });

  test("selected bordered row uses the existing border instead of adding a ring", async () => {
    const row = {
      id: "1",
      original: { id: "1", name: "Alice" },
      getIsSelected: () => true,
