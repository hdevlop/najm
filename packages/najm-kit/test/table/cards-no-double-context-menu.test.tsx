...
describe("cards-no-double-context-menu.test.tsx", () => {
  test("right-clicking a card calls onRowContextMenu exactly once", async () => {
    mockMatchMedia(true);
    const onRowClick = mock();
    const onRowContextMenu = mock();

    function CardComponent(
      { data: row, onContextMenu }: { data: Row; row: any; onClick?: () => void; onContextMenu?: (e: React.MouseEvent) => void; renderSubRow?: (row: Row) => React.ReactNode }
    ) {
      return (
  ...
  test("automatic menu row right-click fires exactly once", async () => {
    mockMatchMedia(true);
    const onSelect = mock();
    ...
    const { container } = render(
      <div style={{ height: 600 }}>
        <NTable
          ...
          mode="cards"
          renderCard={CardComponent}
          menu={{ row: (row: Row) => [{ label: "Copy", onSelect: () => onSelect(row) }] }}
          dynamicHeight={false}
          showPagination={false}
          showAddButton={false}
          showCheckbox={false}
    ...
    const labels = Array.from(menu!.querySelectorAll("[role=menuitem]")).map((b) => (b.textContent ?? "").trim());
