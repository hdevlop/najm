...
| `verifyURL` | `string?` | Verify endpoint URL (default: derived from request origin) |

**Behavior:**
1. Skip public routes
2. Check for refresh token cookie on protected routes
3. If cookie missing: redirect to login with `?from=` param
4. If role-restricted route: verify session server-side and check role
