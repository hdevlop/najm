...
describe('Google OAuth configuration', () => {
  ...
  test('session signing uses explicit config, then NAJM_SESSION_SECRET, then access secret', () => {
    process.env.NAJM_SESSION_SECRET = 'environment-session-secret';
    expect(resolveAuthConfig({ jwt: jwtConfig, encryptionKey }).session.secret)
      .toBe('environment-session-secret');
    expect(resolveAuthConfig({
      jwt: jwtConfig,
      encryptionKey,
      session: { secret: 'explicit-session-secret' },
    }).session.secret).toBe('explicit-session-secret');

    delete process.env.NAJM_SESSION_SECRET;
    const config = resolveAuthConfig({ jwt: jwtConfig, encryptionKey });
    expect(config.session.secret ?? config.jwt.accessSecret).toBe(jwtConfig.accessSecret);
  });
...
describe('Google provider failures', () => {
  ...
  test('maps network, bad status, and non-JSON responses to a stable error', async () => {
    ...
    globalThis.fetch = (async () => new Response('denied', { status: 400 })) as unknown as typeof fetch;
    await expect(provider().exchange('code', attempt)).rejects.toMatchObject({ oauthCode: 'oauth_provider_error' });
