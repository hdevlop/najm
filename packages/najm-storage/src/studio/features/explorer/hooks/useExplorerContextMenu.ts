...
interface Options {
  ...
  onTagFile?: (file: FileItem) => void;
  capabilities?: { tags: boolean };
  bucket?: string;
}

export function useExplorerContextMenu({
  files, selected, dialogs, clipboard, mutations, sortKey, sortDir, onSortChange, onSelectAll, onUploadClick, view, onViewChange, onProperties, onShareFile, onTagFile, capabilities, bucket,
}: Options) {
  ...
  const handleShare = useCallback(async (t: StorageTarget) => {
    if (t.isFolder) {
      toast.info('Folders cannot be shared');
      return;
    }
    ...
    try {
      ...
      if (bucket) {
        const { addSharedLink } = await import('../../../lib/sharedLinks');
        addSharedLink({
          id: `${bucket}/${t.key}/${Date.now()}`,
          url,
          namespace: bucket,
          filePath: t.key,
  ...
  }, [files, mutations, bucket, onShareFile]);

  const handleTag = useCallback((t: StorageTarget) => {
    if (t.isFolder) {
      toast.info('Folders cannot be tagged');
      return;
    }
    const file = files.find((f) => f.filePath === t.key);
    if (!file) {
  ...
  return useStorageContextMenu({
    ...
    itemExtras:        (target) => [
      ...(target.isFolder ? [] : [
        ...(capabilities?.tags ? [{ id: 'tag', label: 'Edit tags', icon: Tag, separatorBefore: true }] : []),
        { id: 'properties', label: 'Properties', icon: Info, separatorBefore: true },
      ]),
      switchViewItem(),
    ],
    ...
    onItemExtraAction: (action, target) => {
      ...
      if (action === 'tag') {
        const file = files.find((f) => f.filePath === target.key);
        if (file && onTagFile) onTagFile(file);
      }
    },
