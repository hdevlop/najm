...
export function useExplorerMutations(bucket: string, refresh: () => void) {
  ...
  const move = useCallback(async (from: string, to: string, options?: { overwrite?: boolean }) => {
    if (from === to) return;
    await api.post(`/${bucket}/files/move`, { from, to, overwrite: options?.overwrite });
  }, [api, bucket]);

  const copy = useCallback(async (from: string, to: string, options?: { overwrite?: boolean }) => {
    if (from === to) return;
    await api.post(`/${bucket}/files/copy`, { from, to, overwrite: options?.overwrite });
  }, [api, bucket]);
