          ...
          <div className="flex gap-1">
            {tags.slice(0, 2).map((tag) => (
              <button
                ...
                className="rounded-md bg-white/10 px-1.5 py-0.5 text-[10px] font-normal text-txt-muted hover:bg-brand/15 hover:text-brand transition-colors cursor-pointer"
              >
                {tag}
              </button>
            ))}
            {tags.length > 2 && (
              <span className="text-[10px] text-txt-muted">+{tags.length - 2}</span>
            )}
          </div>
        );
      },
    ...
    return (
      ...
      >
        ...
        <div className="flex w-full flex-col items-center text-center">
          ...
          {!item.isFolder && (item.tags?.length ?? 0) > 0 && (
            <div className="mt-1 flex flex-wrap justify-center gap-0.5">
              {(item.tags ?? []).slice(0, 2).map((tag) => (
                <button
                  key={tag}
                  onClick={(e) => { e.stopPropagation(); onTagFilter?.(tag); }}
                  className="rounded bg-white/10 px-1 py-px text-[9px] text-txt-muted hover:bg-brand/15 hover:text-brand"
                >
                  {tag}
                </button>
              ))}
              {(item.tags ?? []).length > 2 && (
                <span className="text-[9px] text-txt-muted">+{(item.tags ?? []).length - 2}</span>
              )}
            </div>
          )}
        </div>
