...
export const FileInput: React.FC<FileInputProps> = ({ value, onChange, placeholder = "No file chosen", icon, showIcon = true, iconColor, className = "", variant = "default", status = "default" }) => {
  ...
  return (
    <BaseInput variant={variant} status={status} className={cn("flex h-9 px-0 p-0 text-muted-foreground", className)} onClick={() => fileInputRef.current?.click()}>
