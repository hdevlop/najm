...
export function NTableContent({ effectiveMode }: { effectiveMode?: string }) {
  ...
  return (
    <Card className={cn("rounded-md p-0 border border-border/50 bg-transparent shadow-none", dynamicHeight ? "overflow-hidden" : "overflow-auto", classNames?.content)}>
      {dynamicHeight ? <div className="overflow-auto">{
        <Table>
          <TableHeader className={cn(headerClassName, dynamicHeight && "sticky top-0 z-10", classNames?.tableHeader)}>
            {table.getHeaderGroups().map((hg) => (
              <TableRow key={hg.id} className="hover:bg-muted/30">
                {showCheckbox && (
                  <TableHead className="w-10 text-foreground h-12">
                    <Checkbox
                ...
                {hg.headers.map((header) => (
                  <TableHead key={header.id} className="text-foreground h-12">
          ...
          <TableHeader className={cn(headerClassName, classNames?.tableHeader)}>
            {table.getHeaderGroups().map((hg) => (
              <TableRow key={hg.id} className="hover:bg-muted/30">
                {showCheckbox && (
                  <TableHead className="w-10 text-foreground h-12">
                  ...
                  <React.Fragment key={row.id}>
                    ...
                    >
                      ...
                      {row.getVisibleCells().map((cell) => {
                        ...
                        const meta = columnDef.meta || {};
                        const isEditable = Boolean(onCellEdit) && Boolean(meta.editable);
                        return <TableCell key={cell.id} className="h-14">{isEditable ? <EditableCell cell={cell} onCellEdit={onCellEdit} /> : flexRender(columnDef.cell, cell.getContext())}</TableCell>;
                      })}
                    </TableRow>
                    {isExpanded && renderSubRow && (
                      <TableRow data-expanded-row="true" className="bg-muted/40 hover:bg-muted/40">
