        ...
        {(!isPaginationControlled || manualPagination) && (
          <div className="flex items-center gap-2">
            ...
            <Select value={`${pageSize}`} onValueChange={handlePageSizeChange}>
              ...
              <SelectContent side="top">{pageSizeOptions.map((size) => <SelectItem key={size} value={`${size}`}>{size}</SelectItem>)}</SelectContent>
            </Select>
          </div>
        )}
        <div className="text-sm font-medium">Page {pageIndex + 1} of {effectivePageCount}</div>
        <div className="flex items-center gap-2">
          <Button variant="outline" className="h-8 w-8 p-0 hidden lg:flex" aria-label="First page" onClick={() => navigate("first")} disabled={!table.getCanPreviousPage?.() || pageIndex === 0}><ChevronsLeft className="h-4 w-4" /></Button>
          <Button variant="outline" className="h-8 w-8 p-0" aria-label="Previous" onClick={() => navigate("prev")} disabled={!table.getCanPreviousPage?.() || pageIndex === 0}><ChevronLeft className="h-4 w-4" /></Button>
          <Button variant="outline" className="h-8 w-8 p-0" aria-label="Next" onClick={() => navigate("next")} disabled={!table.getCanNextPage?.() || pageIndex >= effectivePageCount - 1}><ChevronRight className="h-4 w-4" /></Button>
          <Button variant="outline" className="h-8 w-8 p-0 hidden lg:flex" aria-label="Last page" onClick={() => navigate("last")} disabled={!table.getCanNextPage?.() || pageIndex >= effectivePageCount - 1}><ChevronsRight className="h-4 w-4" /></Button>
        </div>
      </div>
