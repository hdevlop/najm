...
import { TextInput, NumberInput, PasswordInput, TextAreaInput, SelectInput, ComboboxInput, MultiSelectInput, RadioGroupInput, SwitchInput, CheckboxInput, CheckboxGroupInput, FileInput, ImageInput, DateInput, StarRatingInput, ColorArrayInput, EmojiInput, LangInput, PhoneInput, TimeInput } from "../inputs";
import { useFormContext } from "react-hook-form";
import { usePrefix } from "./PrefixContext";
import { useVariantPreset } from "./VariantContext";
...
export const FormInput: React.FC<FormInputProps> = ({ name, type, formLabel, formDescription, required = false, disabled = false, hidden = false, icon, iconColor, classNames, ...rest }) => {
    ...
    <FormField control={control} name={fieldName} render={({ field, fieldState }) => {
      ...
      return (
        <FormItem className={slot.item}>
          ...
          <FormControl>
            <InputComponent value={field.value ?? getDefaultValue()} onChange={handleChange} status={hasError && !isHidden ? "error" : "default"} icon={formLabel ? undefined : icon} iconColor={formLabel ? undefined : iconColor} disabled={disabled} {...inputRest} className={slot.input} />
          </FormControl>
          {!hasError && !disabled && formDescription && <FormDescription className={slot.description}>{formDescription}</FormDescription>}
