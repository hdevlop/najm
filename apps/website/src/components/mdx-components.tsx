...
export const mdxComponents = {
  h1: ({ className, ...props }: ComponentProps) => (
    <h1
      className={cn(
        "mt-2 scroll-m-20 text-2xl font-medium tracking-tight text-primary",
        className
      )}
  ...
  h2: ({ className, ...props }: ComponentProps) => (
    <h2
      className={cn(
        "mt-10 scroll-m-20 border-b pb-1 text-xl font-medium tracking-tight first:mt-0",
        className
      )}
      style={{ color: 'rgb(242, 237, 237)' }}
      {...props}
    />
  ),
  ...
  p: ({ className, ...props }: ComponentProps) => (
    <p
      className={cn("text-sm [&:not(:first-child)]:mt-6", className)}
      style={{ color: 'rgb(184, 178, 178)', lineHeight: '1.5' }}
      {...props}
    />
  ),
  ul: ({ className, ...props }: ComponentProps) => (
    <ul className={cn("my-6 ml-6 list-disc text-sm", className)} style={{ color: 'rgb(184, 178, 178)' }} {...props} />
  ),
  ol: ({ className, ...props }: ComponentProps) => (
    <ol className={cn("my-6 ml-6 list-decimal text-sm", className)} style={{ color: 'rgb(184, 178, 178)' }} {...props} />
  ),
  li: ({ className, ...props }: ComponentProps) => (
    <li className={cn("mt-2", className)} style={{ color: 'rgb(184, 178, 178)' }} {...props} />
  ),
